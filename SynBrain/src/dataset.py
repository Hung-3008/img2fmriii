...
def multisub_clip_dataset(args):
    ...
    for subj in subject_list:
        if subj != args.unseen_sub:
            train_fmri = np.load(os.path.join(args.data_path, 'subj0{}/nsd_train_fmri_scale_sub{}.npy'.format(subj, subj))).astype(np.float32)
            print(np.max(train_fmri), np.min(train_fmri))
            train_fmri = train_fmri[:750*args.hour]
            if train_fmri.ndim == 3:
                train_fmri = train_fmri.reshape(-1, train_fmri.shape[-1])
            all_train_fmri[subj] = train_fmri
            print(f'Train Voxel Sub{subj}: {train_fmri.shape}')
            del train_fmri
            
            train_clip_path = os.path.join(args.data_path, 'subj0{}/nsd_train_clip_sub{}.npy'.format(subj, subj))
            ...
            if subj == args.valid_sub:
                if subj in [1, 2, 5, 7]:
                    val_fmri = np.load(os.path.join(args.data_path, 'subj0{}/nsd_test_fmri_scale_sub{}.npy'.format(subj, subj))).astype(np.float32)  #, mmap_mode='r'
                    if val_fmri.ndim == 3:
                        val_fmri = val_fmri.mean(axis=1)
                    else:
                ...
                val_clip_path = os.path.join(args.data_path, 'subj0{}/nsd_test_clip_sub{}.npy'.format(subj, subj))
...
def multisub_clip_test_dataset(args):
    ...
    if subj in [1, 2, 5, 7]:
        val_fmri = np.load(os.path.join(args.data_path, 'subj0{}/nsd_test_fmri_scale_sub{}.npy'.format(subj, subj))).astype(np.float32)
        val_fmri = val_fmri.reshape(-1, 3, val_fmri.shape[1]).mean(axis=1)  #! avg 3 trials
