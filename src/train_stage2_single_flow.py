"""
Stage 2: Single Flow Matching — DINOv2 → fMRI latent.
...
Usage:
    python -m src.train_stage2_single_flow --config src/configs/stage2_single_flow.yaml
    python -m src.train_stage2_single_flow --config src/configs/stage2_single_flow.yaml --debug
"""
...
def validate(model, vae, val_loader, fm, device, ode_steps=50,
    ...
    # --- New Metrics for Vector Field Analysis ---
    all_v_norm = []          # Magnitude of predicted vector field (speed)
    all_ut_norm = []         # Magnitude of True target vector field
    all_traj_length = []     # Total path length traveled by ODE
    all_traj_straight = []   # Straight-line distance between x0 and z_gen
    ...
    for fmri, dino in val_loader:
        ...
        # Flow loss on z1 directly (NO z_bar)
        ...
        # Track vector magnitudes
        all_v_norm.append(torch.norm(v_pred, dim=-1).mean().item())
        all_ut_norm.append(torch.norm(ut, dim=-1).mean().item())

        # ODE generation of z
        ode_fn = ODEWrapper(model, dino, cfg_scale)
        t_span = torch.linspace(0, 1, ode_steps, device=device)
        
        # We only need 1 trial for vector analysis usually, but let's keep the loop to match original
        z_gen = torch.zeros_like(z1)
        traj_len_batch = 0.0
        straight_dist_batch = 0.0
        ...
        for _ in range(num_trials):
            ...
            traj_len_batch += traj_len
            
            # Calculate straight line distance ||z_gen - x0||
            straight_dist = torch.norm(traj[-1] - x0_trial, dim=-1).mean().item()
            straight_dist_batch += straight_dist
            
            z_gen = z_gen + traj[-1]
            
        z_gen = z_gen / num_trials
        all_traj_length.append(traj_len_batch / num_trials)
        all_traj_straight.append(straight_dist_batch / num_trials)
    ...
    metrics = {
        ...
        # newly added
        "val_v_norm": sum(all_v_norm) / len(all_v_norm),
        "val_ut_norm": sum(all_ut_norm) / len(all_ut_norm),
        "val_traj_ratio": (sum(all_traj_length) / len(all_traj_length)) / (sum(all_traj_straight) / len(all_traj_straight) + 1e-8),
...
def main():
    ...
    mse_weight = train_cfg.get("mse_weight", 0.0)
    pcc_weight = train_cfg.get("pcc_weight", 0.0)
    reg_drop_epochs = train_cfg.get("reg_drop_epochs", 0)
    ...
    val_prior_sigma = train_cfg.get("val_prior_sigma", prior_sigma)

    output_dir = cfg.get("output_dir", "results/stage2_single_flow")
    ...
    # Fallback to general config if subject-specific config is missing
    config_path = os.path.join(os.path.dirname(vae_ckpt), "config.yaml")
    if not os.path.exists(config_path):
        config_path = "src/configs/fmri_mlp_vae_768_subj02.yaml"
        logger.info(f"VAE config not found at dir, using {config_path}")
    ...
    fields = [
        "epoch", "train_loss", "flow_loss", "mse_loss", "pcc_loss", "lr",
        "grad_avg", "grad_max",
        "val_flow_loss", "val_v_cos", "val_v_norm", "val_ut_norm", "val_traj_ratio",
    ...
    if reg_drop_epochs > 0:
        logger.info(f"Regression warmup: {reg_drop_epochs} epochs")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        ep_reg, ep_flow, ep_mse, ep_pcc, ep_total, n_steps = 0, 0, 0, 0, 0, 0
        ...
        for batch_idx, (fmri, dino) in enumerate(train_loader):
            ...
            # ── x1-Prediction Auxiliary Loss ──────────────────────────────────
            # z1_pred = xt + (1-t)*v_pred  →  push model to correctly predict z1
            # This directly reduces MSE without breaking flow properties
            t_expand = t[:, None] if t.ndim == 1 else t
            z1_pred = xt + (1 - t_expand) * v_pred

            loss_mse = F.mse_loss(z1_pred, z1) if mse_weight > 0 else torch.tensor(0.0, device=device)

            if pcc_weight > 0:
                z1_pred_zm = z1_pred - z1_pred.mean(dim=1, keepdim=True)
                z1_zm = z1 - z1.mean(dim=1, keepdim=True)
                cos_sim = F.cosine_similarity(z1_pred_zm, z1_zm, dim=1).mean()
                loss_pcc = 1.0 - cos_sim
            else:
                loss_pcc = torch.tensor(0.0, device=device)

            loss = flow_weight * loss_flow + mse_weight * loss_mse + pcc_weight * loss_pcc
            ...
            ep_mse += loss_mse.item() if isinstance(loss_mse, torch.Tensor) else loss_mse
            ep_pcc += loss_pcc.item() if isinstance(loss_pcc, torch.Tensor) else loss_pcc
            ep_total += loss.item()
        ...
        logger.info(
            ...
            f"flow={avg_flow:.5f} mse={avg_mse:.5f} pcc={avg_pcc:.5f} | "
        ...
        if epoch % eval_interval == 0 or epoch == 1:
            ...
            val = validate(eval_model, vae, val_loader, fm, device,
                           ode_steps, cfg_scale, num_trials, val_prior_sigma,
            ...
            row = {"epoch": epoch,
                   ...
                   "pcc_loss": f"{avg_pcc:.6f}",
                   "lr": f"{current_lr:.2e}",
                   "grad_avg": f"{avg_grad:.4f}",
                   "grad_max": f"{max_grad:.4f}",
                   **{k: f"{v:.6f}" if (
                       'loss' in k or 'mse' in k or 'std' in k or 'norm' in k or
            ...
            logger.info(
                ...
                f"v_nrm={val['val_v_norm']:.4f} ut_nrm={val['val_ut_norm']:.4f} "
                f"traj_R={val['val_traj_ratio']:.4f} | "
                f"L-MSE={val['val_latent_mse']:.4f} "
                f"L-PCC={val['val_latent_pcc']:.4f} | "
                f"F-MSE={val['val_fmri_mse']:.4f} "
                f"F-sPCC={spcc:.4f} | "
                f"z-std={val['val_zgen_std']:.4f}"
                f"{'  ★' if is_best else ''}")
