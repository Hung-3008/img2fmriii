"""
Stage 2 Training: Multi-Layer DINOv2 Conditional Flow Matching.

Extends train_stage2.py with:
    - Multi-layer DINOv2 features (B, 4, 257, 1024)
...
Usage:
    ...
    python -m src.train_stage2_multilayer --config src/configs/stage2_multilayer.yaml --debug
"""
...
from src.model.multilayer_flow_sit import (
    MultiLayerFlowSiT, MultiLayerFlowSiTConfig)
...
class FmriMultiLayerDataset(Dataset):
    ...
    DINOv2: (N, n_layers, 257, 1024) — 4 layers of full tokens.
    fMRI: average 3 reps → (N, 15724).
    """
...
def main():
    parser = argparse.ArgumentParser(
        "Stage 2: Multi-Layer DINOv2 Flow Matching")
    ...
    logger = logging.getLogger('stage2_ml')
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch_idx, (fmri, dino) in enumerate(train_loader):
            ...
            if batch_idx == 0 and epoch <= 5:
                ...
                logger.info(
                    f"  [Ep{epoch} B0] flow={loss.item():.4f} "
                    f"v_cos={v_cos:.4f}")
        ...
        if epoch % eval_interval == 0 or epoch == 1:
            ...
            logger.info(
                ...
                f"f_spcc={spcc:.4f} | "
                f"zgen: std={val['val_zgen_std']:.4f} "
                f"var_ratio={val['val_zgen_crossvar_ratio']:.4f}"
                f"{'  ★' if is_best else ''}")
