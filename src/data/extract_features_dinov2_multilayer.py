...
=====================================
Extracts multi-layer DINOv2 ViT-L/14 features from PNG images.

Uses `model.get_intermediate_layers()` to extract tokens from
layers [6, 12, 18, 24] of DINOv2 ViT-L/14 (24 layers total).

Output shape: (N, 4, 257, 1024)  float16
  - 4 layers × (1 CLS + 256 patches) × 1024-dim

Storage: ~19 GB per train split, ~2 GB per test split.

Usage:
  python SynBrain/data/extract_features_dinov2_multilayer.py
  python SynBrain/data/extract_features_dinov2_multilayer.py --subjects 1
"""
...
DEFAULT_BATCH_SIZE = 64  # smaller due to multi-layer memory
DINOV2_EMB_DIM = 1024
...
# ViT-L/14 has 24 blocks (0-23). These correspond to layers 6,12,18,24 (1-indexed)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Extract multi-layer DINOv2 ViT-L/14 features")
        ...
        for mode in ['train', 'test']:
            ...
            for batch_i, images in enumerate(dataloader):
                ...
                with torch.no_grad(), torch.amp.autocast(
                    ...
                    # Stack layers → (B, n_layers, 257, 1024)
            ...
            if all_features:
                ...
                out_path = os.path.join(
                    save_path,
                    f'nsd_dinov2_vitl14_multilayer_{mode}_sub{subj}.npy')
